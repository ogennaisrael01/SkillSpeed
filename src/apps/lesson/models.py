...
class LessonContent(models.Model):
    ...
    content_url  = models.URLField(null=True, blank=True) # for file and video contents
    content_body = models.TextField(null=True, blank=True) # for text content 

    content_status = models.CharField(choices=ContentStatus.choices, default=ContentStatus.IN_PROGRESS)
...
class Progress(models.Model):
    progress_id = models.UUIDField(max_length=20, primary_key=True, unique=True, default=uuid.uuid4)

    child_profile = models.ForeignKey(ChildProfile, on_delete=models.CASCADE, related_name="progress")
    lesson_content = models.ForeignKey(LessonContent, on_delete=models.CASCADE, related_name="progress")
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(fields=["child_profile", "lesson_content"], name="unique_user_content")
        ]
...
class Projects(models.Model):
    ...
    project_id = models.UUIDField(primary_key=True, unique=True, default=uuid.uuid4, max_length=20)
    lesson_content = models.ForeignKey(LessonContent, on_delete=models.CASCADE, related_name="projects")
    ...
    difficulty = models.CharField(choices=ProjectDifficulty.choices, default=None)
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(fields=['lesson_content', 'title'], name='unique_project_title_per_lesson_content')
        ]
    def __str__(self):        
...
class Submission(models.Model):
    ...
    submission_id = models.UUIDField(primary_key=True, unique=True, db_index=True,
                                     max_length=20, default=uuid.uuid4)

    project = models.ForeignKey(Projects, on_delete=models.SET_NULL, null=True, blank=True, related_name="submissions")
    child_profile = models.ForeignKey(ChildProfile, on_delete=models.CASCADE, related_name="submissions")

    submission_type = models.CharField(choices=SubmissionType.choices, default=None)
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(fields=['project', 'child_profile'], name='unique_submission_per_project_per_user')
        ]
