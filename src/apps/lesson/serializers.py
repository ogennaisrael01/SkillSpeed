...
class LessonCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            "content_body", "content_order", "is_published"
        ]
    ...
    def validate(self, attrs):
        ...
        if content_type not in content_type_choices:
            raise serializers.ValidationError(
                _(f"Invalid content type. Must be one of: {', '.join(content_type_choices)}"
                  ))

        if content_type in [
                LessonContent.ContentType.VIDEO, LessonContent.ContentType.FILE
        ]:
            if not attrs.get("content_url"):
                raise serializers.ValidationError(
                    _("content_url is required for VIDEO and FILE content types."
                      ))
            if not validate_urls(attrs["content_url"]):
                raise serializers.ValidationError(
                    _("Invalid URL format for content_url."))

        if content_type == LessonContent.ContentType.TEXT:
            if not attrs.get("content_body"):
                raise serializers.ValidationError(
                    _("content_body is required for TEXT content type."))
        return attrs

    def create(self, validated_data):
        if user is None:
            raise serializers.ValidationError(
                _("Auth credentials were not provided"), code="login_required")
...
class LessonReadSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = LessonContent
        fields = [
            "skill", "title", "description", "content_type", "content_url",
            "content_body", "content_status", "content_order", "current_level",
            'is_active', "is_completed", "is_published", "created_at",
            "completed_at"
        ]
...
class ProjectSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Projects
        fields = [
            "lesson_content", "description", "title", "instructions",
            "description", "difficulty", "is_required", "start_date",
            "end_date", "created_at", "duration"
        ]
        read_only_fields = ["lesson_content", "created_at", "duration"]

    default_error_messages = {
        "invalid_dates":
        _("end_date must be greater than start_date."),
        "detailed_instruction":
        _("Please give a detailed instruction of this project.")
    }
...
class SubmissionCreateSerializer(serializers.ModelSerializer):
    ...
    default_error_messages = {
        "no_submission_type":
        _("Please provide accurate submission type."),
        "no_file_content":
        _("Please provide the accurate file content"),
        "no_text_content":
        _("Please provide accurate text content for this submission")
    }
    ...
    def create(self, validated_data):
        ...
        try:
            with transaction.atomic():
                Submission.objects.create(project=project,
                                          child_profile=child_profile,
                                          **validated_data)
                Progress.objects.update_or_create(
                    lesson_content=project.lesson_content,
                    child_profile=child_profile,
                    is_completed=True)
        except Exception as e:
            raise serializers.ValidationError(
                _(f"Error creating submissions: {str(e)}"),
    ...
    def update(self, instance, validated_data):
        ...
        try:
            ...
            raise serializers.ValidationError(
                _(f"Error updating request: {str(e)}"), code="invalid_request")
...
class FeebBackSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Submission
        fields = ["feedback", "work_rating", "instructor_points"]
    ...
    def validate_instructor_points(self, value):
        ...
        if not 10 <= value <= 100:
            raise serializers.ValidationError(
                _("Instructor point must be between 10 and 100."))
        return value
    ...
    def update(self, instance, validated_data):
        ...
        if current_date > project_end_date:  # project is expired
            instance.total_points_after_validatiokn = validated_data[
                "instructor_points"] - 5  # minus 5 points for late submission
        try:
            with transaction.atomic():
            ...
            raise serializers.ValidationError(
                _(f"error seeding feedback: {str(e)}"), code="invalid_request")
        return instance
...
class AcceptRejectProjectSerializer(serializers.ModelSerializer):
    ...
    default_error_messages = {
        "invalid_value":
        _("Invalid status value. Must be either 'APPROVED' or 'REJECT'."),
        "can_update":
        _("Invalid request. You can only approve or reject already submitted projects"
          )
    }
    ...
    def update(self, instance, validated_data):
        ...
        try:
            ...
            raise serializers.ValidationError(
                f"Error updating status: {str(e)}", code="request_rejected")
        return instance
...
class SubmissionReadSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = serializers
        fields = [
            "project", "child_profile", "submission_type", "submission_text",
            "submission_file", 'status', "feedback", "work_rating",
