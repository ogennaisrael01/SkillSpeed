...
from .helpers import (can_access_content, user_current_level,
                      _get_contents_for_user, get_current_status,
                      create_progress_record, get_content_by_pk,
...
class LessonContentViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if skill_pk is None or qs is None:
            ...
            qs = qs.filter(skill__pk=skill_pk,
                           is_active=True,
                           is_published=True)
        return qs
        ...
        return Response(
            {
                "status": "success",
                "detail": {
                    "message": "skill content added successfully",
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        if not content_obj.is_active:
            return Response({
                "status": "error",
                "message": "This content is not verified"
            })
        user_level = user_current_level(request,
                                        getattr(content_obj.skill, "pk"))
            ...
            return Response(
                {
                    ...
                    "detail": {
                        "message":
                        "You have not unlocked this content yet. Please complete previous contents to unlock it."
                    }
                },
            ...
            return Response(
                {
                    ...
                    "detail": {
                        "message":
                        "You are not enrolled in this skill. Please enroll to access this content."
                    }
                },
                status=status.HTTP_403_FORBIDDEN)
            ...
            return Response(
                {
                    ...
                    "detail": {
                        "message":
                        "This content does not belong to the skill you are enrolled in."
                    }
                },
                status=status.HTTP_403_FORBIDDEN)
            ...
            return Response(
                {
                    ...
                    "success",
                    "detail":
                    "This content has already been completed, you can continue with the next content"
                },
                status=status.HTTP_200_OK)
        try:
            ...
            return Response(
                {
                    ...
                    "detail": {
                        "message":
                        f"An error occurred while marking content as completed: {str(e)}"
                    }
                },
...
class SubmissionViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if qs is None:
            ...
            qs = qs.filter(
                Q(child_profile=self.request.user.active_account)
    ...
    def list(self, request, *args, **kwargs):
        ...
        if qs is None:
            return Response(
                {
                    "status": "not_found",
                    "message": "no response for your request"
                },
    ...
    def feedback(self, request, *args, **kwargs):
        ...
        serializer = FeebBackSerializer(instance,
                                        data=request.data,
                                        partial=True)
    ...
    def approve_reject_project(self, request, *args, **kwargs):
        instance = self.get_object()
        serializer = AcceptRejectProjectSerializer(instance,
                                                   data=request.data,
                                                   partial=True)
