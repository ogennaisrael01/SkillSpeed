...
class CategorySerializer(serializers.ModelSerializer):
    ...
    default_error_messages = {
        "invalid_category":
        _("The category you provided is invalid. lookout for allowed roles"),
    ...
    class Meta:
        model = SkillCategory
        fields = [
            "category_id", "name", "descriptions", "icon", "is_active",
            "created_at"
        ]
        read_only_fields = ["is_active", "category_id", "created_at"]
...
class SkillCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Skills
        fields = [
            "name", "description", "skill_difficulty", "min_age", "max_age",
            "price", "is_paid"
        ]
    ...
    def validate(self, attrs):
        ...
        if min_age is None or max_age is None:
            raise serializers.ValidationError(
                _("Please provide the age limit for this skill"),
                code="provide_age")
        if max_age > 15 or min_age < 5:
            raise serializers.ValidationError(
                _("'min age' cannot be below 5 and 'max_age' cannot be above 15"
                  ))
        name = attrs.get("name")
    ...
    def create(self, validated_data):
        ...
        if is_paid and price is None or price and not is_paid:
            raise serializers.ValidationError(_(
                "price cannot be empty when is paid is marked true or otherwise"
            ),
        ...
        if getattr(request.user, "user_role") != User.UserRoles.INSTRUCTOR:
            raise serializers.ValidationError(
                _("You cannot perform this action"), code="invalid_reuqest")
        if not isinstance(category, SkillCategory):
            raise serializers.ValidationError(
                _("'category is not a valid skillcategory obj"))
        with transaction.atomic():
            Skills.objects.create(user=request.user,
                                  category=category,
                                  **validated_data)
...
class SkillReadSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Skills
        fields = [
            "skill_id", "category", "user", "name", "price", "description",
            "min_age", "max_age", "is_paid", "is_active", "created_at",
            "skill_difficulty"
        ]
...
class EnrollmentSerializer(serializers.Serializer):
    ...
    def validate_email(self, value):
        if value is None:
            raise serializers.ValidationError(_(
                "provide guardian email address used when registring account"),
                                              code="invalid_request")
        email = value.strip()
        try:
    ...
    def validate(self, attrs):
        ...
        user = getattr(request, "user")
        if Enrollment.objects.filter(child_profile=child_profile,
                                     skill=skill,
                                     is_active=True).exists():
            raise serializers.ValidationError(
                _('you already enrolled for this skill'))
        if child_profile not in user.children.all():
            raise serializers.ValidationError(
                _("Invalid request, you cannot perform this action"))
        child_age = _child_age_or_none(child_profile=child_profile)
        if child_age is None:
            raise serializers.ValidationError(
                _("Child age cannot be empty, its helps in giving you the best"
                  ))
    ...
    def create(self, validated_data):
        ...
        user = getattr(request, "user")
        if skill.is_paid:
            payment = get_object_or_404(Purchase,
                                        skill=skill,
                                        purchased_by=user,
            ...
            if payment.price != skill.price:
                raise serializers.ValidationError(
                    "payment price doesen't match")
            with transaction.atomic():
                Enrollment.objects.create(skill=skill,
...
class EnrollmentReadSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Enrollment
        fields = [
            "child_profile", "skill", "is_active", "created_at", "enrol_id"
        ]
