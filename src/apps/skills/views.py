...
class SkillSearchView(ListAPIView):
    ...
    def get_queryset(self):
        return Skills.objects.select_related("user", "category").filter(is_active=True, is_deleted=False)
    ...
    def get(self, request, *args, **kwargs):
        ...
        return Response({"status": "success", "message": "No skills found matching the query"},
...
class SkillsViewSet(viewsets.ModelViewSet):
    ...
    filterset_fields = ["category__name", "min_age", "max_age", "name", "price"]
    pagination_class = CustomSkillPagination
    ...
    def get_queryset(self):
        ...
        if "query" in self.request.query_params:
            ...
            qs = qs.filter(Q(category__name__iexact=query.upper()) |
                            ...
                            Q(min_age__gte=query) |
                            Q(max_age__lte=query) 
                           , is_active=True, is_deleted=False)
            return qs
        else:
            qs = qs.filter(category__pk=category_pk, is_active=True, is_deleted=False)
            return qs
    ...
    def create(self, request, *args, **kwargs):
        ...
        if category_pk is None:
            return Response({"status": "success", "message": "category pk is not provided"}, 
                            status=status.HTTP_400_BAD_REQUEST)

        category_instance = get_object_or_404(SkillCategory, pk=category_pk.strip(), is_active=True)
        serializer = self.get_serializer(data=request.data, context={"request": request, 
                                                                     "category": category_instance})
        valid_serializer = _validate_serializer(serializer=serializer)
        self.perform_create(valid_serializer)
        return Response({"status": "success", "message": "Skill added successfully",
                         "detail": {"user": request.user.email, "category": category_instance.py}},
