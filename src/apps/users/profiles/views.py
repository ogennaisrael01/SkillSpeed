...
from .serializers import (OnboardSerializer, ChildProfileCreateSerializer,
                          ...
                          ChildReadSerializer, InterestSerializer,
                          CertificateSerializer, RoleSwitchSerializer)
from ..helpers import _validate_serializer
from .permissions import (IsGuardian, IsAdminOrInstructor, IsOwner,
                          ChildProfileOwner, ChildRole, IsInterestOwner,
                          IsInstructor)
...
class ProfileManagementViewsets(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if super_user or self.reqeust.user.user_role == User.UserRoles.INSTRUCTOR:
            guardian = Guardian.objects.select_related("user").filter(
                is_active=True)
    ...
    def list(self, request, *args, **kwargs):
        ...
        if guardian_paginator is not None and instructor_paginator is not None:
            guardian_serializer = GuardianProfileSerializer(guardian_paginator,
                                                            many=True)
            instructor_serialzer = InstructorSerializer(instructor_paginator,
                                                        many=True)
    ...
    def onboard(self, request, *args, **kwargs) -> Response:
        ...
        valid_serializer: ListSerializer | transaction.Any | OnboardSerializer = _validate_serializer(
            serializer=serializer)
        self.perform_create(serializer=valid_serializer)
        return Response({
            "status": "success",
            "detail": valid_serializer.data
        },
                        status=status.HTTP_200_OK)
    ...
    def child_onboard(self, request, *args, **kwargs) -> Response:
        ...
        return Response(
            {
                "status": "success",
                "detail": valid_serializer.validated_data
            },
        ...
        return Response(
            {
                ...
                "detail": {
                    ...
                    "active_role": request.user.active_profile
                }
            },
            status=status.HTTP_200_OK)
...
class ChildProfileManagement(viewsets.ModelViewSet):
    ...
    queryset = ChildProfile.objects.select_related(
        "guardian").prefetch_related("interest")
...
class InterestViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return self.queryset.filter(child=child,
                                    is_active=True,
                                    is_deleted=False)
    ...
    def perform_destroy(self, instance):
        if not isinstance(instance, ChildInterest):
            return Response(
                {
                    "status": "failure",
                    "detail": "'instance is not a valid child interest object'"
                },
                status=status.HTTP_400_BAD_REQUEST)
...
class CertificatedViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        user_profile = self.request.user.profile if hasattr(
    ...
    def create(self, request, *args, **kwargs):
        ...
        profile_instance = get_object_or_404(Instructor,
                                             pk=instructor_profile,
                                             is_active=True)
        serializer = self.get_serializer(data=request.data,
                                         context={
                                             "request": request,
...
class SwithBetweenChildAccountView(APIView):
    ...
    def patch(self, request, *args, **kwargs):
        child_pk = kwargs.get("child_pk")
        status, child_profile = child_in_guardian_account(
            request.user, child_pk)
        if not status:
            raise PermissionDenied(
                "You dont have permission to switch to this child account",
        ...
        return Response({
            "status": "success",
            "data": {
                "child_id": child_profile.pk,
                "name":
                child_profile.first_name + "  " + child_profile.last_name,
