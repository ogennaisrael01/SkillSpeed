...
from .helpers import (_validate_email, _check_email_already_exists,
                      _normalize_and_validate_password, _get_user_by_email,
                      _get_code, _get_reset_code_or_none)
...
class UserRegistrationSerializer(serializers.ModelSerializer):
    confirm_password = serializers.CharField(
        max_length=200,
        write_only=True,
        error_messages={"required": _("provide confirm password")})
    ...
    def validate(self, attrs: dict) -> dict:
        ...
        if password != confirm_password:
            raise serializers.ValidationError(
                _("Password mismatch. Please provide a matching password"))
        for field in ("first_name", "last_name"):
            value = attrs.get(field)
            if not attrs[field] or value is None:
                raise serializers.ValidationError(
                    _(f"{value} cannot be empty..."))
    ...
    def validate_email(self, value: str):
        ...
        if user:
            raise serializers.ValidationError(
                _(f"user with {user} already exists"))
...
class UserVerificationSerializer(serializers.Serializer):
    ...
    def validate_code(self, value):
        if not isinstance(value, str):
            raise serializers.ValidationError(
                _("%s is not a valid str instance", value))
        return value
    ...
    def validate(self, attrs):
        ...
        if email is None or code is None:
            raise serializers.ValidationError(
                _("'email' or code is required to valaidate account"))
        if _check_email_already_exists(email):
            raise serializers.ValidationError(
                "Account already verified and ready for login")
        ...
        if user is None or one_time_password is None:
            raise serializers.ValidationError(
                _("Invalid code or email provided"))

        if not one_time_password.is_active or one_time_password.is_used:
            raise serializers.ValidationError(
                "Password already expired or used. Request another one")
        return attrs
