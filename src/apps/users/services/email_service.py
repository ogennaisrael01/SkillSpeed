...
def _send_mail_base(context: dict) -> bool:
    ...
    if SENDGRID_SENDER is None:
        logger.warning(
            "'SENDGRID_SENDER' sender is empty, authenticate in sendgrid dashboard"
        )
    required_fiedls = ("email", "subject", "template_name")
    if (context.get(field) for field in required_fiedls) is None:
        raise ValidationError(
            "email context is required. provide accurate payload body")
    ...
    try:
        ...
        message = Mail(from_email=From(email=SENDGRID_SENDER),
        ...
        if response.status_code != 202:
            ...
            raise ValidationError(
                f"Email failed with status: {response.status_code}")
    except KeyError:
